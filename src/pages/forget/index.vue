...
<div class="loginLayer">
    <login-header currentTitle="找回密码"/>
    <div class="content">
        <Form :model="form" ref="LoginForm" :rules="form.rules" class="loginbox custom-form ">
            ...
            <FormItem prop="password">
                <Input v-model="form.password" type="password" class="input-text" placeholder="密码">
                    <Icon slot="prepend" type="ios-locked"></Icon>
                </Input>
            </FormItem>
            <FormItem>
                <Button type="primary" class="btn-login" @click.prevent="handleLogin('LoginForm')">找回密码</Button>
            </FormItem>
...
<script>
    import { AjaxForgetPassword, AjaxSendSMS, AjaxgetCaptcha } from 'src/apis/user'
        ...
        methods:{
                        ...
                        self.$refs[name].validateField('picCode', (error) => {
                            if(!error){
                                ...
                                AjaxSendSMS(data).then(res => {
                                    ...
                                    //     self.$Message.success(res.message? res.message : '成功！')
                                    // }else{
            ...
            handleLogin(name){
                ...
                this.$refs[name].validate((valid) => {
                    if(valid) {
                        let data = {
                            mid: self.form.username,
                            phone: self.form.phone,
                            smscode: self.form.code,
                            newpwd: self.form.password
                        }
                        AjaxForgetPassword(data).then(res => {
                            if(res.status === 'success'){
